...
현재 `classic-v2`는 초기 5글자 가림을 최소화하고 성장량을 원작의 40%로 조정했다. 최근 미세 조정으로 직전 버전보다 성장량을 20% 높였다. 가림막 표시 길이는 최대 120px이며 브라우저와 Rust 서버가 같은 규칙을 사용한다. 원본 대비 변경 수치는 [검토 문서](docs/PROJECT_REVIEW.md)에 기록한다.
