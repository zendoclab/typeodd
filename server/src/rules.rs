...
impl Game {
    ...
    pub fn input(&mut self, value: &str, now: u64) -> bool {
        ...
        if correct {
            ...
            if self.width < 250. {
                self.width += self.instantaneous_cpm / 100.;
            }
        ...
        self.veil = if self.width.trunc() > 39. {
...
mod tests {
    ...
    fn original_trace() {
        ...
        assert_eq!((g.width, g.veil, g.score), (43., 19, 22));
        g.input("X", 800);
        assert_eq!((g.width, g.veil, g.score), (1., 17, 15));
        g.input("", 900);
        assert_eq!((g.width, g.veil, g.score), (1., 20, 10));
        g.input("i", 1100);
        assert_eq!((g.width, g.veil, g.score), (2.5, 23, 11));
    }
    #[test]
