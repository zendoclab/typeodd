...
const GROWTH_MULTIPLIER: f64 = 1.5;
    ...
    pub fn input(&mut self, value: &str, now: u64) -> bool {
        ...
        if correct {
            ...
            let ramp = ((self.completed_chars + 1).saturating_sub(WARMUP_CHARS) as f64
                ...
                .min(1.);
            let growth =
                self.instantaneous_cpm.min(MAX_GROWTH_CPM) / GROWTH_DIVISOR * GROWTH_MULTIPLIER;
...
mod tests {
    ...
    fn warmup_and_batched_input_are_bounded() {
        ...
        for i in 0..100 {
            let before = g.width;
            g.input("a", 0);
            assert!(g.width - before <= 5.4 + 1e-9);
            assert!(g.width <= 80.);
            if i < 5 {
                assert_eq!(g.width, 1.);
            }
